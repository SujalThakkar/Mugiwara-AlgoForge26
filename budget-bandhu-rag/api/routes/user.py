"""
User Routes - Registration, Login, Profile (Mobile Number ID)
BudgetBandhu API
"""
...
async def register_user(user_data: UserCreate, db=Depends(get_database)):
    """
    Register a new user using Mobile Number.
    Auto-creates default budget and gamification profile.
    """
    users_collection = db["users"]
    
    # Check if mobile already exists
    # Mobile is _id, so we check by _id
    existing = await users_collection.find_one({"_id": user_data.mobile})
    if existing:
        raise HTTPException(status_code=400, detail="Mobile number already registered")
    
    # Create user document
    user_doc = {
        "_id": user_data.mobile, # PRIMARY KEY
        "name": user_data.name,
    ...
    budget_doc = {
        ...
        "allocations": [a.dict() for a in default_allocations],
    ...
    print(f"[USER] Registered: {user_id} ({user_data.name})")
...
async def login_user(login_data: UserLogin, db=Depends(get_database)):
    ...
    # Find by mobile (_id)
    user = await users_collection.find_one({"_id": login_data.mobile})
    if not user:
    ...
    print(f"[USER] Login: {login_data.mobile}")
