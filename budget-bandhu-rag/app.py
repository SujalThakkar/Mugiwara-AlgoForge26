...
from intelligence import ml_client
...
async def categorize(request: CategorizeRequest):
    try:
        response = await ml_client.categorize([request.description])
        result = response["results"][0] if "results" in response and response["results"] else {"category": "Other"}
        return {"success": True, **result}
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
...
async def forecast(request: ForecastRequest):
    ...
    try:
        result = await ml_client.get_forecast(request.user_id)
        return {"success": True, **result}
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
...
async def anomaly_check(request: AnomalyCheckRequest):
    try:
        txn = {"description": request.description, "amount": request.amount, "category": request.category or "Other"}
        response = await ml_client.detect_anomalies([txn])
        result = response["results"][0] if "results" in response and response["results"] else {"is_anomaly": False}
        return {"success": True, **result}
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
...
async def add_transaction(request: TransactionRequest):
    ...
    try:
        ...
        if not request.category:
            cat_resp = await ml_client.categorize([request.description])
            category = cat_resp["results"][0]["category"] if "results" in cat_resp and cat_resp["results"] else "Other"
        else:
            category = request.category
            
        # 2. Anomaly
        anom_resp = await ml_client.detect_anomalies([{
            "description": request.description,
            "amount": request.amount,
            "category": category
        }])
        anomaly = anom_resp["results"][0] if "results" in anom_resp and anom_resp["results"] else {"is_anomaly": False, "reason": "Unknown"}
        ...
        return {
            ...
            "category": category,
            "is_anomaly": anomaly.get("is_anomaly", False),
            "warning": anomaly.get("reason") if anomaly.get("is_anomaly") else None
        }
    except Exception as e:
